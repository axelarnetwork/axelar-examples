...
async function createLocal(toFund = []) {
    async function callback(chain, info) {
        ...
        await chain.deployToken('Wrapped BTC', 'WBTC', 8, BigInt(1e70));
        await chain.deployToken('SushiToken', 'SUSHI', 18, BigInt(1e70));

        for (const address of toFund) {
            await chain.giveToken(address, 'aUSDC', BigInt(1e18));
            await chain.giveToken(address, 'WBTC', BigInt(1e18));
            await chain.giveToken(address, 'SUSHI', BigInt(1e18));
        }
    }
