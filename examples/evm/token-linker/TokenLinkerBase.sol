...
abstract contract TokenLinkerBase is AxelarExecutable, Upgradable {
    ...
    function sendToken(string calldata destinationChain, address to, uint256 amount, address refundAddress) external payable virtual {
        ...
        if (gasValue > 0) {
            gasService.payNativeGasForContractCall{ value: gasValue }(address(this), destinationChain, thisAddress, payload, refundAddress);
        }
