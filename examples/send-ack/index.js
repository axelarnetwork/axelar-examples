...
const SendAckReceiver = require('../../build/SendAckReceiverImplementation.json');
const SendAckSender = require('../../build/SendAckSender.json');
...
async function test(chains, wallet, options) {
    ...
    async function print() {
        ...
        console.log(`SendAckReceiverImplementation at ${destination.name} has ${length} messages and the last one is "${length>0 ? await destination.receiver.messages(length-1) : ''}".`)
    }
    function sleep(ms) {
    ...
    const tx = await (await source.sender.sendContractCall(
        destination.name,
